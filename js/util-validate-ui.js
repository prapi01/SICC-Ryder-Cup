...
KEY CHANGES from v1.15:
   - CHANGED: renderHandicapAdjustmentCard() table now shows "Raw New" and "Final New" columns
   - CHANGED: Removed "Anc" and "Perf" columns (now showing raw/final handicap comparison)
   - CHANGED: Uses recalculated rawNew and finalNew (with zero rise applied) from validation result
   - FIXED: Display now clearly shows the before/after zero rise values
   - PRESERVED: All existing rendering functions from v1.15
...
var UtilValidateUI = (function() {
    ...
    console.log("[UTIL-VALIDATE-UI] Initializing v1.16 - Raw New / Final New columns");
    ...
    // ============================================================
    // v1.16: RENDER: Handicap Adjustment Card (Raw New / Final New)
    // ============================================================
    ...
    function renderHandicapAdjustmentCard(validationResult, containerId) {
        ...
        // v1.16: Get zero rise info
        var zeroRiseAmount = recalculated.zeroRiseAmount || 0;
        var needsZeroRise = recalculated.needsZeroRise || false;
        var newAnchor = storedHandicaps.newAnchor || 'None';
        ...
        // v1.16: Summary line with zero rise info
        var newAnchorDisplay = formatNewAnchorDisplay(newAnchor);
        var zeroRiseDisplay = needsZeroRise && zeroRiseAmount !== 0 ? '+' + zeroRiseAmount : 'None';
        ...
        html += '<span>Zero Rise: <strong style="color:#4caf50;">' + zeroRiseDisplay + '</strong></span>';
        if (summary.totalFields > 0) {
            html += '<span style="margin-left:auto;">✅ ' + summary.matched + ' ❌ ' + summary.mismatched + '</span>';
        }
        html += '</div>';
        
        // v1.16: Table with Raw New and Final New columns
        ...
        html += '<th style="padding:3px 3px; text-align:center; font-size:0.6rem; color:#ffaa44; font-weight:700;">Raw New</th>';
        html += '<th style="padding:3px 3px; text-align:center; font-size:0.6rem; color:#4caf50; font-weight:700;">Final New</th>';
        ...
        html += '<td style="padding:2px 3px; text-align:center; font-size:0.5rem; color:#ffaa44;">Before Zero Rise</td>';
        html += '<td style="padding:2px 3px; text-align:center; font-size:0.5rem; color:#4caf50;">After Zero Rise</td>';
        html += '<td style="padding:2px 3px; text-align:center; font-size:0.5rem; color:#555;"></td>';
        html += '</tr>';
        ...
        for (var i = 0; i < players.length; i++) {
            ...
            if (p.team !== currentTeam) {
                ...
                html += '<td colspan="5" style="padding:4px 3px; text-align:center; color:#4caf50; font-weight:600; font-size:0.65rem;">' + teamLabel + '</td>';
                html += '</tr>';
            }
            
            // v1.16: Get values for Raw New and Final New comparison
            var startingHcp = p.startingHcp;
            var rawNew = recalc ? recalc.rawNew : undefined;
            var finalNew = recalc ? recalc.finalNew : undefined;
            var storedFinalHcp = stored ? stored.finalHcp : undefined;
            
            // Determine if match
            var isMatch = false;
            if (storedFinalHcp !== undefined && finalNew !== undefined) {
                if (typeof storedFinalHcp === 'number' && typeof finalNew === 'number') {
                    isMatch = Math.abs(storedFinalHcp - finalNew) < 0.01;
                } else {
                    isMatch = storedFinalHcp === finalNew;
                }
            }
            
            if (!isMatch) hasMismatch = true;
            
            // Check if this player is the anchor
            var isAnchor = stored && storedHandicaps.anchor === p.name;
            var isNewAnchor = stored && storedHandicaps.newAnchor === p.name;
            
            // Format values
            var displayRawNew = (rawNew !== undefined && rawNew !== null && !isNaN(rawNew)) ? rawNew : '-';
            var displayFinalNew = (finalNew !== undefined && finalNew !== null && !isNaN(finalNew)) ? finalNew : '-';
            var displayStored = (storedFinalHcp !== undefined && storedFinalHcp !== null && !isNaN(storedFinalHcp)) ? storedFinalHcp : '-';
            
            // Color coding
            var rawColor = '#ffaa44';
            var finalColor = isMatch ? '#4caf50' : '#ff6b6b';
            var stColor = isAnchor ? '#ffaa44' : '#ffffff';
            
            var rowBg = isMatch ? '#0a0a0a' : '#1a0a0a';
            ...
            html += '<td style="padding:4px 3px; text-align:center; color:' + stColor + ';">' + startingHcp + '</td>';
            
            // Raw New (before zero rise)
            var rawDisplay = (rawNew !== undefined && rawNew !== null && !isNaN(rawNew)) ? rawNew : '-';
            var rawExtra = '';
            if (rawNew !== undefined && rawNew !== null && !isNaN(rawNew) && rawNew < 0) {
                rawExtra = ' <span style="color:#ff6b6b; font-size:0.5rem;">⬇</span>';
            }
            html += '<td style="padding:4px 3px; text-align:center; color:' + rawColor + '; font-weight:600;">' + rawDisplay + rawExtra + '</td>';
            
            // Final New (after zero rise)
            var finalDisplay = (finalNew !== undefined && finalNew !== null && !isNaN(finalNew)) ? finalNew : '-';
            var finalExtra = '';
            if (isNewAnchor) {
                finalExtra = ' <span style="color:#ffaa44; font-size:0.5rem;">⭐</span>';
            }
            html += '<td style="padding:4px 3px; text-align:center; color:' + finalColor + '; font-weight:700;">' + finalDisplay + finalExtra + '</td>';
            
            // Status
            var statusIcon = isMatch ? '✅' : '❌';
            var statusColor = isMatch ? '#4caf50' : '#ff6b6b';
            html += '<td style="padding:4px 3px; text-align:center; color:' + statusColor + '; font-weight:600;">' + statusIcon;
            if (!isMatch && storedFinalHcp !== undefined && finalNew !== undefined) {
                html += ' <span style="color:#888; font-size:0.5rem; display:block;">exp:' + finalNew + '</span>';
            }
            html += '</td>';
    ...
    function renderValidateResults(recordData, validation, validateGameData, validateCurrentValidation) {
        ...
        // v1.16: Render Handicap Adjustment card with Raw New / Final New columns
...
console.log('[UTIL-VALIDATE-UI] v1.16 - Raw New / Final New columns');
