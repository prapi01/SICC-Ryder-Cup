...
> ⚠️ **Device identity (Phase 1 finding, 2026-08-09):** `SessionManager.getShortDeviceName()` allocated `DEV-01..DEV-99` short names by scanning `deviceMapping`; mappings accumulated forever, so once all 99 names were taken a new device's loop did ~99 sequential Firestore queries (~2 min) and `pre-game.html` appeared to hang (no role buttons). **APP-SIDE FIX IMPLEMENTED — `js/session.js` v1.02 (2026-08-09):** one bulk read of the small collection + in-memory scan, stale mappings (`lastSeen` > 30 days) pruned in the background, and a bounded reap of the least-recently-seen mapping when full — allocation is now O(1) network calls. The harness still **pre-seeds each context's `localStorage.deviceId` + `shortDeviceName`** (defense-in-depth; also avoids writing new mappings). Monitor accumulation with `automated-tests/tools/probe-device-mapping.js` / `simulate-allocation.js`.
